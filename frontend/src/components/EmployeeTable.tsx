...
function EmployeeTable({ orgid }: { orgid: string; }) {
	const { data: employees, isPending: isGettingEmployees,error } = useGetEmployees(orgid as string);
  ...
  return (
    				<div className="flex md:flex-row flex-col justify-center gap-5 items-center h-full w-full mt-2">
									...
									{employees?.map((employee:Emp) => {
										return <TableRow key={employee._id}>
											<TableCell className="font-medium flex gap-5 ml-2">
												<UserAvatar customSeed={employee.user.avatar || employee.user.email}
												className="w-10 h-10 "
												/>
